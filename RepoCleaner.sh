...
delete_old_repo() {
  if test -d "$REPO"; then
    ...
    sudo rm -rf "$REPO" || { echo "Failed to delete repo"; exit 1; }
  fi
}
...
wipe_info() {
  java -jar bfg-1.14.0.jar --replace-text passwords.txt "$REPO" \
    || { echo "Failed to wipe info"; exit 1; }
  cd "$REPO" || { echo "Failed to change directory"; exit 1; }
  git reflog expire --expire=now --all \
    || { echo "Failed to expire reflog"; exit 1; }
  git gc --prune=now --aggressive \
    || { echo "Failed to garbage collect"; exit 1; }
}

update_repo() {
  git push || { echo "Failed to update repo"; exit 1; }
}
